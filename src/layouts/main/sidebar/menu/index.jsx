    ...
    <nav className="mt-0.5 mb-1" key={account}>
      {mainMenu.map((menu, index) => (
        <NavLink key={index} to={typeof menu.path === 'function' ? menu.path() : menu.path}
          ...
          {({isActive}) => (
            <div
              className={classNames("p-3 rounded-full transition-colors inline-flex items-center group-hover:bg-[color:var(--background-third)]", {
                  "font-bold": isActive
                }
              )}>
              <div className="w-[26.25px] h-[26.25px] relative">
                {menu?.notification && (
                  <span
                    className="bg-[color:var(--color-primary)] w-[1.125rem] h-[1.125rem] rounded-full absolute -right-1 -top-1.5 flex justify-center text-[11px] ">
                    {menu?.notification}
                </span>
                )}
